...
// Strip arm/shoulder/hand tracks from any GLB clip so only spine/torso sway
// survives into the mixer. Face morphs + procedural gaze handle lips/head;
// arms rest at the GLB bind pose — no floaty clip drift, no geometric override needed.
...
export function OracleAvatar3D({ visemeStateRef, cameraStateRef, seekerMotionRef }: OracleAvatar3DProps) {
  ...
  const talk1Anim = useMemo(() => stripHeadNeck(talking1Clips).map(stripArmTracks), [talking1Clips]);
  const talk2Anim = useMemo(() => stripHeadNeck(talking2Clips).map(stripArmTracks), [talking2Clips]);
  ...
  // Eases so the body never snaps between clips.
