...
import { DIDWebRTCClient } from './DIDWebRTCClient';
...
export const SurrogateOracleImmersion: React.FC = () => {
  ...
  // Initialize Oracle (D-ID WebRTC)
  const initializeOracle = async () => {
    if (!validateEnvironment()) return;
    if (!idleVideoRef.current || !streamVideoRef.current) {
      setOracleState(prev => ({ ...prev, error: 'Oracle video elements not ready' }));
      return;
    }
    ...
    try {
      console.log('🚀 Initializing SURROGATE Oracle via D-ID WebRTC...');
      ...
      didClient.current.setCallbacks({
        ...
        onTalkStarted: () => {
          console.log('🎤 Oracle speaking (D-ID)...');
          setOracleState(prev => ({ ...prev, isProcessing: true }));
        },
        onTalkEnded: () => {
          console.log('🎤 Oracle finished speaking (D-ID)');
      ...
      const result = await didClient.current.initializeStream(
        idleVideoRef.current,
        streamVideoRef.current
      );

      if (!result?.success) {
        throw new Error(result?.error || 'Failed to initialize D-ID stream');
      }

      console.log('✅ Oracle (D-ID) stream initialized');
    } catch (error: any) {
  ...
  // NOTE: OracleConversation provides text response for D-ID TTS
  const handleOracleResponse = async (oracleTextResponse: string) => {
    if (!didClient.current?.isStreamActive()) return;

    try {
      console.log('🎤 Sending text to D-ID for TTS and lip-sync:', oracleTextResponse);
      const result = await didClient.current.sendTalk(oracleTextResponse);

      if (!result.success) {
        const errorMsg = `D-ID TTS error: ${result.error}`;
        console.error('❌', errorMsg);
        setOracleState(prev => ({ ...prev, error: errorMsg }));
      } else {
        console.log('✅ Text sent to D-ID successfully');
        setOracleState(prev => ({ ...prev, error: null }));
      }
    } catch (error: any) {
      const errorMsg = `Oracle/D-ID response error: ${error.message}`;
      console.error('❌', errorMsg);
      setOracleState(prev => ({ ...prev, error: errorMsg }));
    }
  };
  ...
  const exitOracleMode = async () => {
    ...
    if (didClient.current) {
      await didClient.current.closeStream();
    }
  ...
  return (
    <div className="oracle-immersion-container">
      ...
      >
        {/* Static Oracle image (visible until D-ID stream is active) */}
        <img 
        ...
        <video
          ...
          style={{
            ...
            objectFit: 'cover',
            borderRadius: '15%',
            opacity: isOracleMode && !oracleState.isProcessing ? 1 : 0,
        ...
        {isOracleMode && (
          ...
          >
            ...
            <OracleConversation
              ...
              // IMPORTANT: OracleConversation must call this with text response
              // for D-ID's built-in TTS
