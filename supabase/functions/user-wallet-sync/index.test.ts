...
type FakeSupabaseOptions = {
  synchronizeChallengeUpdates?: boolean;
};
...
function fakeSupabase(
  ...
  let nonce = 0;
  let pendingChallengeUpdates = 0;
  let releaseChallengeUpdates: (() => void) | null = null;
  const challengeUpdatesReady = options.synchronizeChallengeUpdates
    ? new Promise<void>((resolve) => {
        releaseChallengeUpdates = resolve;
      })
    : null;
  ...
  const client = {
    from(table: string) {
      ...
      const builder = {
        ...
        async maybeSingle() {
          if (operation === "update" && challengeUpdatesReady) {
            pendingChallengeUpdates += 1;
            if (pendingChallengeUpdates === 2) releaseChallengeUpdates?.();
            await challengeUpdatesReady;
          }
          ...
          const matches = challenges.filter((challenge) =>
            Object.entries(filters).every(([column, expected]) => {
              if (
                ...
                return (
                  challenge[column as keyof Challenge] ===
                  (expected as { is: unknown }).is
                );
              }
              if (
                typeof expected === "object" &&
                ...
                return (
                  new Date(
                    String(challenge[column as keyof Challenge]),
                  ).getTime() >
...
Deno.test(
  "accepts a valid challenge signature and writes wallet identity",
  async () => {
...
Deno.test(
  "rejects missing and malformed identity proof without writing",
  async () => {
    for (const body of [
      {
        action: "upsert",
...
Deno.test(
  "rejects a mismatched wallet, expired challenge, and replay",
  async () => {
    const wallet = Wallet.createRandom();

    const mismatched = fakeSupabase();
    const mismatchedHandler = createWalletSyncHandler(
      mismatched.client as never,
      () => NOW,
    );
    const { message } = await issueChallenge(mismatchedHandler, mismatched);
    const signature = await wallet.signMessage(message);
    const wrongCallerResponse = await mismatchedHandler(
      request(
        {
          action: "upsert",
          onboarding_status: "wallet_signed",
    ...
    const mismatchResponse = await mismatchedHandler(
      request({
        action: "upsert",
        onboarding_status: "wallet_signed",
        wallet_address: Wallet.createRandom().address,
    ...
    const expiredHandler = createWalletSyncHandler(
      expired.client as never,
      () => NOW,
    );
    ...
    const replayHandler = createWalletSyncHandler(
    ...
    assertMatch(
      String((await responseBody(replayResponse)).error),
      /already used/,
    );
...
Deno.test(
  "allows only one of two concurrent submissions to consume a challenge",
  async () => {
    const store = fakeSupabase([], { synchronizeChallengeUpdates: true });
    const handler = createWalletSyncHandler(store.client as never, () => NOW);
    const { message } = await issueChallenge(handler, store);
    const wallet = Wallet.createRandom();
    const payload = {
    ...
    assertEquals(bodies.filter((body) => body.success === true).length, 1);
    assertEquals(
      bodies.filter(
        (body) =>
          body.success === false && /already used/.test(String(body.error)),
      ).length,
      1,
    );
    assertEquals(store.writes.length, 1);
    assertEquals(store.challenges[0].used_at !== null, true);
  },
);

Deno.test(
  "allows lifecycle status writes without identity signatures",
  async () => {
    const store = fakeSupabase();
    const handler = createWalletSyncHandler(store.client as never, () => NOW);
    ...
    for (const status of ["visited", "lore_completed"]) {
      const response = await handler(
        request({ action: "upsert", onboarding_status: status }),
      );
      assertEquals(response.status, 200);
    }
    ...
    assertEquals(
      store.writes.map((write) => write.p_status),
      ["visited", "lore_completed"],
    );
    assertEquals(
      store.writes.every((write) => write.p_wallet_address === null),
      true,
    );
  },
);
