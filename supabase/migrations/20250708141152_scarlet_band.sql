/*
  # Fix Culture Coin System Migration

  1. New Tables
    - Makes all table creation conditional with IF NOT EXISTS
    - Makes data insertion idempotent with ON CONFLICT DO NOTHING
    
  2. Functions
    - Recreates functions with OR REPLACE
...
-- Create culture_coin_levels table if it doesn't exist
CREATE TABLE IF NOT EXISTS culture_coin_levels (
...
-- Create oracle_interactions table to track each conversation
...
-- Create oracle_subscriptions table
...
-- Add data to culture_coin_levels using ON CONFLICT to handle existing records
...
-- Check if RLS is already enabled before enabling it
DO $$
BEGIN
  -- Enable RLS for culture_coin_levels if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'culture_coin_levels' 
    AND rowsecurity = true
  ) THEN
    ALTER TABLE culture_coin_levels ENABLE ROW LEVEL SECURITY;
  END IF;
  
  -- Enable RLS for user_consciousness_metrics if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'user_consciousness_metrics' 
    AND rowsecurity = true
  ) THEN
    ALTER TABLE user_consciousness_metrics ENABLE ROW LEVEL SECURITY;
  END IF;
  
  -- Enable RLS for oracle_interactions if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'oracle_interactions' 
    AND rowsecurity = true
  ) THEN
    ALTER TABLE oracle_interactions ENABLE ROW LEVEL SECURITY;
  END IF;
  
  -- Enable RLS for oracle_subscriptions if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'oracle_subscriptions' 
    AND rowsecurity = true
  ) THEN
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE tablename = 'culture_coin_levels' 
    AND policyname = 'Anyone can read levels'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'oracle_interactions' 
    AND policyname = 'Users can manage their interactions'
  ) THEN
...
-- Create indices for performance if they don't already exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_oracle_interactions_user_id'
  ) THEN
    CREATE INDEX idx_oracle_interactions_user_id ON oracle_interactions(user_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_oracle_interactions_session_id'
  ) THEN
    CREATE INDEX idx_oracle_interactions_session_id ON oracle_interactions(session_id);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_oracle_interactions_category'
  ) THEN
    CREATE INDEX idx_oracle_interactions_category ON oracle_interactions(question_category);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_metrics_level'
  ) THEN
    CREATE INDEX idx_user_metrics_level ON user_consciousness_metrics(current_level);
  END IF;
  
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_metrics_subscription'
  ) THEN
    CREATE INDEX idx_user_metrics_subscription ON user_consciousness_metrics(subscription_tier);
  END IF;
END
$$;