/*
# Fix policy exists error

1. Tables
   - Drops and recreates culture_coin_levels table
   - Creates user_consciousness_metrics, oracle_interactions, and oracle_subscriptions tables if they don't exist

2. Functions
   - Creates functions for classifying questions and calculating culture coins
   - Creates a stored procedure for processing Oracle interactions

3. Policies
   - Adds conditional policy creation to check if policies already exist
   - Enables RLS on all tables

4. Indices
   - Creates performance optimization indices
*/
...
-- Check if policy exists before creating it for culture_coin_levels
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polrelid = 'culture_coin_levels'::regclass 
    AND polname = 'Anyone can read levels'
  ) THEN
...
-- Check if policy exists before creating it for user_consciousness_metrics
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polrelid = 'user_consciousness_metrics'::regclass 
    AND polname = 'Users can view own metrics'
  ) THEN
...
-- Check if policy exists before creating it for oracle_interactions
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polrelid = 'oracle_interactions'::regclass 
    AND polname = 'Users can manage their interactions'
  ) THEN
...
-- Check if policy exists before creating it for oracle_subscriptions
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policy 
    WHERE polrelid = 'oracle_subscriptions'::regclass 
    AND polname = 'Users can view own subscription'
  ) THEN
