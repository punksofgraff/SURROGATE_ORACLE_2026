/*
  # Culture Coin System Migration - Fixed uid() error

  1. Schema Updates
    - Creates culture_coin_levels table for user progression
    - Ensures user_consciousness_metrics table exists
    - Ensures oracle_interactions table exists
    - Ensures oracle_subscriptions table exists
    
  2. Data Population
    - Populates culture_coin_levels with progression levels
    
  3. Functions
    - Creates classify_oracle_question function for content analysis
    - Creates calculate_culture_coins function for rewards
    - Creates process_oracle_interaction function for full interaction workflow
    
  4. Security
    - Enables RLS on all tables
    - Sets up appropriate policies with auth.uid() instead of uid()
...
DROP POLICY IF EXISTS "Users can manage their metrics" ON user_consciousness_metrics; 
...
-- Add broader policy for user_consciousness_metrics
...
-- FIXED: Changed uid() to auth.uid() for Supabase compatibility
